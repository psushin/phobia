/* Secret Knock Trinket
   ...
   ------Wiring------
   Pin 2: Record A New Knock button.
   Pin 13: (uses the built in LED)
   Pin 14 (Analog 1): A piezo element for beeping and sensing knocks.
   Pin 8: Connects to a transistor that opens a solenoid lock when HIGH.
*/
...
/*Pin definitions*/
const char programButton = 2;   // Record A New Knock button.
const char ledPin = 13;          // The built in LED
const char knockSensor = 14;     // (Analog 1) for using the piezo as an input device. (aka knock sensor)
const char audioOut = 9;        // (Digial 2) for using the peizo as an output device. (Thing that goes beep.)
const char lockPin = 8;         // The pin that activates the solenoid lock.
...
void setup() {
  ...
  doorUnlock(350);     // Unlock the door for a bit when we power up. For system check and to allow a way in if the key is forgotten.
...
void doorUnlock(int delayTime){
  ...
  delay(10);					// эту задержку не менять
